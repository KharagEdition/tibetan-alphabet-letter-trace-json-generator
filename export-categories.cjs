...
/* Rebuild the `categories` array inside letters.json/letters.js, and the
   ...
   Run this after editing letters.json by hand (e.g. adding a new letter or
   changing a letter's roles) so the category groupings stay in sync.
*/
'use strict';
const fs = require('fs');
...
const DEFS = [
  ...
  { id: 'prefixes',       file: 'prefix-letters',        label: 'Prefix Letters',       match: L => (L.roles || []).includes('prefix') },
  { id: 'superscribed',   file: 'superscribed-letters',  label: 'Superscribed Letters', match: L => (L.roles || []).includes('superscribed') },
  { id: 'subscribed',     file: 'subscribed-letters',    label: 'Subscribed Letters',   match: L => L.category === 'subscribed' },
  { id: 'suffixes',       file: 'suffix-letters',        label: 'Suffix Letters',       match: L => (L.roles || []).includes('suffix') },
  { id: 'secondSuffixes', file: 'second-suffixes',       label: 'Second Suffixes',      match: L => (L.roles || []).includes('secondSuffix') }
];
...
for (const c of doc.categories) console.log(` ${c.id}: ${c.count} letter(s)`);
